...
use crate::messages::{OBlock, Timeout, Vote, QC, TC};
...
impl Core {
    ...
    async fn store_block(&mut self, block: &OBlock) {
    ...
    async fn make_vote(&mut self, block: &OBlock) -> Option<Vote> {
    ...
    async fn commit(&mut self, block: OBlock) -> ConsensusResult<()> {
    ...
    async fn handle_proposal(&mut self, block: &OBlock) -> ConsensusResult<()> {
